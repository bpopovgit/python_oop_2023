...
class TestCarManager(TestCase):
    ...
    def test_zero_value_fuel_consumption_raises_exception(self):
        with self.assertRaises(Exception) as ex:
            car = Car("Nissan", "GT-R", 0, 75)

        self.assertEqual("Fuel consumption cannot be zero or negative!", str(ex.exception))

    def test_zero_value_fuel_capacity_raises_exception(self):
        with self.assertRaises(Exception) as ex:
    ...
    def test_refuel_zero_fill_raises_exception(self):
        with self.assertRaises(Exception) as ex:
            self.car.refuel(0)
